...
tools: Use any tools necessary for reviewing Flutter/Dart code changes
---
...
- **Skip** stylistic preferences unless they violate project lint rules or Effective Dart.
- **Skip** issues in unchanged code unless they are CRITICAL security issues.
- **Consolidate** similar issues (e.g., 10 missing `const` constructors → one finding).
- **Prioritize** logic bugs, security holes, state management errors, and test coverage gaps.
...
### Coverage targets:
- [ ] Aim for **95%+ line coverage** on business logic (project-wide standard)
...
File: lib/screens/profile_screen.dart:88
Issue: Navigator.push called after await without checking context.mounted.
Fix: Add `if (!context.mounted) return;` before the navigation call.
```
...
Verdict: BLOCK — 2 HIGH issues (missing const constructors, mutable state in provider) must be resolved.

## Suggested Next Steps

- Call **tdd-guide** to write missing widget and unit tests.
- Call **security-reviewer** for deeper security audit on API key handling.
- Call **refactor-cleaner** to extract oversized build methods.
```
